...
function UsuarioCrudController() {
    ...
    self.idade = ko.observable("0");
    self.DataNascimento.subscribe(d => self.calcularIdade(d))

    self.temUsuarios = () => self.usuarios().length > 0

    self.carregarUsuariosDoBackend = function () {
        $.get("/UsuarioJson/List", resp => self.usuarios(resp.data))
    }

    self.usuarioHandler = function (usuario) {
        try { usuario.DataNascimento = usuario.DataNascimento.jsonToDate() }
        catch (e) { }
        return usuario
    }

    self.usuarios.subscribe(function (usuarios) {
        usuarios.forEach(u => u = self.usuarioHandler(u))
    })
    ...
    self.editar = function (usuario) {
        ...
        self.DataNascimento(usuario.DataNascimento.format("yyyy-mm-dd")) 
    }
    ...
    self.salvar = function () {
        ...
        self.erros.removeAll()
        self.updateUsuario(novoUsuario);
        self.limpaUsuario();
    }


    self.updateUsuario = function (novoUsuario) {
        const velhoUsuario = self.usuarios().find(c => c.Id == novoUsuario.Id);
        console.log(velhoUsuario)
        if (velhoUsuario != null) {
            self.calcularIdade(self.dataCliente)
            $.post("/UsuarioJson/Edit", novoUsuario, function (resp) {
                if (resp.success) {
                    self.usuarios.replace(velhoUsuario, novoUsuario)
                } else {
                    self.erros(resp.errors)
                }
            })

        } else {
            $.post("/UsuarioJson/add", novoUsuario, function (resp) {
                self.calcularIdade(self.dataCliente)
                if (resp.success) {
                    self.add(resp.data)
                } else {
    ...
    self.calcularIdade = function (DataNascimento) {
        var hoje = new Date();
    ...
    self.remover = function (usuario) {
        $.get("/UsuarioJson/Delete/" + usuario.Id, function (resp) {
            if (resp.success) {
...
const _usuarioCrudController = new UsuarioCrudController()
ko.applyBindings(_usuarioCrudController)
$(function () { _usuarioCrudController.carregarUsuariosDoBackend() })